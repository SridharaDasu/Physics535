 ...
 *      Author: adotti
 * @brief  Implements user class for analysis
 */
...
{
  if (histos.size()>3) {
    if (aTrack->GetDefinition()->GetPDGEncoding()!=-11) return; // positron
    /*    
G4String particleName = aTrack -> GetDefinition()->GetParticleName();
    G4int PDG = aTrack->GetDefinition()->GetPDGEncoding();

    G4cout << particleName << "      "  << PDG << G4endl;
    */
...
void Analysis::PrepareNewRun(const G4Run* /*aRun*/ )
{
	...
	TH1D *h=0;
	// create Histograms
	histos.push_back(h=new TH1D("decayPos","Z Position of Decay",100,0.8,(0.8+2.24)) );
	h->GetYaxis()->SetTitle("events");
	h->GetXaxis()->SetTitle("z_{decay} / m");
	h->StatOverflows();
