...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  //Construction of the magnetic field
  //COMMENT FOR THE Task1c.3
  ConstructField(); 
...
G4VPhysicalVolume* DetectorConstruction::ConstructEMCalo()
{
  ...
  G4LogicalVolume* emLogic = new G4LogicalVolume( emSolid,//its solid
						  pbw04, //its material; Define as air for the Task1c.3
...
// Optionally change the visalisation attribute to better identify the part of interest  
  red = new G4VisAttributes(G4Colour(1., 0., 0.));
  red-> SetVisibility(true);
  red -> SetForceWireframe(true);
  
  emLogic -> SetVisAttributes(red);

  
  //Now we create the central volume that will be embedded in the previous volume
  G4Box* emCentralSolid = new G4Box("emCentralSolid", 
				    halfEmCentralSizeX,
				    halfEmCentralSizeY,
				    halfEmCaloSizeZ);

  G4LogicalVolume* emCentralLogic = new G4LogicalVolume(emCentralSolid,
							pbw04,// Define as air for the Task1c.3
							"emCentralLogic");
  ...
  G4Color red(1,0,0);
  emCentralLogic->SetVisAttributes(new G4VisAttributes(yellow));
...
G4VPhysicalVolume* DetectorConstruction::ConstructHadCalo()
{
  ...
  G4LogicalVolume* hadCaloLogic = 
    new G4LogicalVolume( hadCaloSolid,//its solid
			 fe,//its material
