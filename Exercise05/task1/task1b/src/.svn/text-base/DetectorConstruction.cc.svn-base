...
void DetectorConstruction::ComputeParameters() 
{
  ...
  posSimpleBox  = G4ThreeVector(0., 0., -300*mm - emCaloLength);
...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
 ...
 // Costruction of a simple geometric configuration
  ConstructBox();
...

{
  ...
  //-------------------------
  // Construct a box with name "simpleBox" and half lengths pX,pY,pZ respectively 20*mm, 
  //  20*mm,20*mm
  // Edit the following line:
  // G4Box* simpleBox = G4Box(const G4String& pName, G4double pX, G4double pY, G4double pZ);
  
  
  G4LogicalVolume* SimpleBoxLogic = 0;
  //-------------------------
  //Exercise 2 of task1b
  //-------------------------
  // Create a logical volume from the previous solid, using material = fe
  // Edit the following line:
  // SimpleBoxLogic = 
  //   G4LogicalVolume(G4VSolid* its_solid ,G4Material* its_material,const G4String& its_name);

  // Set visualization attributes for the logical volume
  ...
  //-------------------------
  // Create a physical volume with the following settings: 
  // no rotation (0),its traslation (posSimpleBox), lits ogical volume (SimpleBoxLogic), 
  // its name (SimpleBoxPl), its mother volume logicWorld, copy number(10),no boolean operation
