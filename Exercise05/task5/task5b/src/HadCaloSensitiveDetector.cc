...
G4bool HadCaloSensitiveDetector::ProcessHits(G4Step *step, G4TouchableHistory *)
{
  ...
  //If the step does not have energy depostion (for example because the particle is
...
void HadCaloSensitiveDetector::Initialize(G4HCofThisEvent* HCE)
{
  ...
  if (!collectionName.size())
    {  
      G4cout << "Aborting execution" << G4endl;
      G4Exception("HadCaloSensitiveDetector::Initialize()","task5b_1",
		  FatalException,
		  "Please complete exercise 5b.1, otherwise the application will crash");
    }  
  ...
  // strings: the name of the SD and the name of the collection. Uncomment and 
  ...
  // Hint 2: Get the collection_name from the collectionName vector: 
  //         your collection is at position 0: collectionName[0]
   
  // 
  // Now we can attach this collection to the "Hits Collection of this Event":
