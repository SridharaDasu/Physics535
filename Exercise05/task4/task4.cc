...
int main(int argc,char** argv)
{
  ...
  G4int verbose=0;
  //Here we are using a Geant4 ready-for-the-use physics list
  //called QGSP_BERT.
  G4VUserPhysicsList* physics = new QGSP_BERT(verbose);
  //In principle, one could use a custom (local) physics list by
  //implementing a derived class of G4VUserPhysicsList*. This 
  //example contains a physics list class called PhysicsList().
  //One can use it instead of the QGSP_BERT one --> see following line
  // G4VUserPhysicsList* physics = new PhysicsList();
  runManager->SetUserInitialization(physics);

  // The command below is used to set the verbosity for the hadronic 
  // processes, e.g. how much information you get on screen
  // -------------------------------------------------------
  // Task 4.1
  // -------------------------------------------------------
  ...
  //Stacking Action
  //Notice: the optional Stacking Action class is *not* registered 
  //to the RunManager, so it is actually unused.
  // -------------------------------------------------------
  // Task 4.3
  // -------------------------------------------------------
